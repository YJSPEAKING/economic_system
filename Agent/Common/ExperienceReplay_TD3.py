...
class Experience_Replay:
    ...
    def ra_record(self, h_epi, state, reward_pro, reward=None, final_state=None):
        if is_list(h_epi):
            ep_num = len(h_epi)
            ret = [None] * ep_num
            for i in range(ep_num):
                s = state[i] if is_list(state) else state
                a = reward_pro[i] if is_list(reward_pro) else reward_pro
                r = reward[i] if is_list(reward) else reward
                fs = final_state[i] if is_list(final_state) else final_state
                ret[i] = self.ra_record(h_epi[i], s, a, r, fs)
            return ret
        if h_epi is None: h_epi = -1
        if reward is None: reward = 0
        if final_state is None:
        ...
        h_epi = _ex_rp_record(self._kernel, LH_EPI_T(h_epi), state, ACTION_T(reward_pro),
                              REWARD_T(reward), final_state, STATE_SIZE_T(state_size))
        self._mutex.unlock()

        return h_epi

    def get_random_batch_ra(self, batch_size:int, valid_sample_rate:float = 0, h_ps = None):
    ...
    def get_random_batch_lstm(self, batch_size:int, valid_sample_rate:float = 0, h_ps = None, a_dim = 0):
        if h_ps is None: h_ps = H_PS_T(0)
        self._mutex.lock()
        if self._state_shape is None:
            self._mutex.unlock()
        ...
        batch = dict(s = state[:,pick_len-1,:], # 将每个batch中最后一个state取出，组成[batch,s_dim]的数组
                     s_ = state_[:,pick_len-1,:],
                     a = real_action[:,pick_len-1,:],
                     r = reward[:,pick_len-1],
                     h_s = state[:,:pick_len-1:],
                     h_a = real_action[:,:pick_len-1:],
                     h_s_ = state_[:,:pick_len-1:],
                     h_a_ = real_action[:,1:pick_len:],
                     h_s_len = h_s_len
                     )
        # return [state, real_action, reward, state_, seq_len, seq_len_, pick_epi, pick_pos]
        return batch
