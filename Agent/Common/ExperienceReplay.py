...
class Experience_Replay:
    ...
    def record(self, h_epi, state, action, reward = None, final_state = None):
        ...
        # print("action",action)
        if self.isFour and len(action[0]) > 1:
            action = action_encode(action)
        else:
            if self.isFloat:
                action = float2int16(action)
        # print("encode action ",action)
    ...
    def get_random_batch(self, batch_size:int, valid_sample_rate:float = 0, h_ps = None):
        ...
        if self.isFour:
            for i in range(len(action)):
                real_action.append(action_decode(action[i][0]))
        else:
            if self.isFloat:real_action=int_list2float(action,self.isFour)
        # print("after batch",real_action)
