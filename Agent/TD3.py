...
class TD3(object):
    def __init__(self,config:Config):
        ...
        self.disc_update_every = max(1, getattr(config, 'DISC_UPDATE_EVERY', 5))
    ...
    def learn(self):
        ...
        if (not tranLock) or self.pointer < self.var_end_at:
            ...
            use_gail = hasattr(self, 'gail_disc') and hasattr(self, 'sample_expert')
            if use_gail:
                fake_s_n = b_s_tensor
                fake_a_n = (b_a_tensor - self.act_mean) / torch.sqrt(self.act_var + 1e-8)

            if use_gail and self.update_cnt % self.disc_update_every == 0:
                # 开启循环“加练”模式
                for _ in range(disc_update_ratio):
                    ...
                    if expert_s is not None:
                        ...
                        loss_D_real = F.binary_cross_entropy_with_logits(
                            real_logits,
                            torch.full_like(real_logits, self.disc_real_label)
                        )
                        loss_D_fake = F.binary_cross_entropy_with_logits(
                            fake_logits,
                            torch.full_like(fake_logits, self.disc_fake_label)
                        )
