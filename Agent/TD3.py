...
class Actor(nn.Module):
    ...
    def forward(self,state, hist_state=None):
        ...
        if self.use_history and hist_state is not None:
            ...
            h = torch.cat([state, hist_feat], dim=1)
            h = F.tanh(self.hl1(h))
            h = F.leaky_relu(self.hl2(h))
            return self.a_bound * torch.tanh(self.hl3(h))
...
class TD3(object):
    ...
    def choose_action(self, h_epi, state):
        ...
        '''
        actor_hist = self._actor_history_tensor(temp, state)
        list = [self.actor(state.reshape(1, -1), actor_hist)]
        action = list[0][0]
        self._remember_actor_state(temp, state)
        if isPercent:
            action = action.cpu().detach() + np.random.normal([0 for i in range(self.a_dim)], self.var)
            for i in range(len(action)):
                if action[i] > self.a_bound:
    ...
    def _actor_history_tensor(self, temp, state):
        ...
        return torch.as_tensor(np.stack(window, axis=0)[np.newaxis, :, :], dtype=torch.float32, device=device)
    ...
    def _replace_last_history_action(self, hist_action, action):
        if hist_action is None:
            return None
        hist_action = hist_action.clone()
        hist_action[:, -1, :] = action
        return hist_action
    ...
    def learn(self):
        ...
        if self.is_rms:
            ...
            b_s_rm = b_M[0]
            b_s__rm = b_M[3]
        batch_size = np.array(b_M[0]).shape[0]
        pick_len = np.array(b_M[0]).shape[1]
        b_s_seq = np.array(b_s_rm).reshape(batch_size, pick_len, self.s_dim)
        b_s_next_seq = np.array(b_s__rm).reshape(batch_size, pick_len, self.s_dim)
        b_s = b_s_seq[:, -1, :]

        # 根据网络设定获取当前状态b_s，动作b_a，奖励b_r，下一个状态b_s_的数据，进行数据处理
        b_a_seq = np.array(b_M[1]).reshape(batch_size, pick_len, self.a_dim)
        b_a = b_a_seq[:, -1, :]
        b_r_seq = np.array(b_M[2]).reshape(batch_size, pick_len)
        b_r = b_r_seq[:, -1].reshape(-1, 1)
        b_s_next = b_s_next_seq[:, -1, :]
        b_not_done = self._make_not_done_mask(b_M, b_s_next)
        ...
        b_s_ = torch.as_tensor(b_s_next, dtype=torch.float32, device=device).detach()
        ...
        b_s_hist_tensor = torch.as_tensor(b_s_seq, dtype=torch.float32, device=device).detach()
        b_a_hist_tensor = torch.as_tensor(b_a_seq, dtype=torch.float32, device=device)
        fake_s_n = b_s_tensor
        fake_a_n = b_a_tensor
        fake_s_seq_n = b_s_hist_tensor
        fake_a_seq_n = b_a_hist_tensor
        if hasattr(self, 'act_mean') and hasattr(self, 'act_var'):
            fake_a_n = (b_a_tensor - self.act_mean) / torch.sqrt(self.act_var + 1e-8)
            fake_a_seq_n = (b_a_hist_tensor - self.act_mean) / torch.sqrt(self.act_var + 1e-8)
        ...
        if (not tranLock) or self.pointer < self.var_end_at:
            ...
            if hasattr(self, 'gail_disc') and hasattr(self, 'sample_expert'):
                ...
                for _ in range(disc_update_ratio):
                    ...
                    if expert_seq is not None:
                        ...
                        fake_logits = self.gail_disc(fake_s_seq_n, fake_a_seq_n)
            ...
            with torch.no_grad():
                ...
                next_actor_hist = self._next_state_history(b_s_hist_tensor, b_s_)
                target_action = self.actor_target(b_s_, next_actor_hist)
                if self.is_smooth:
                ...
                # 计算 target Q 值
                next_critic_hist_s = next_actor_hist
                next_critic_hist_a = self._next_action_history(b_a_hist_tensor, action_with_noise)
                target_Q1, target_Q2 = self.critic_target(b_s_, action_with_noise, next_critic_hist_s, next_critic_hist_a)
            ...
            if self.update_cnt % self.policy_target_update_interval == 0:
                ...
                self.actor_loss = -self.critic.Q1(b_s_tensor, actor_action, b_s_hist_tensor, actor_hist_a).mean()
        ...
        # 只计算critic 的loss 不进行网络更新
            ...
            with torch.no_grad():
                ...
                # 计算target Q 值
                next_critic_hist_a = self._next_action_history(b_a_hist_tensor, action_with_noise)
                target_Q1,target_Q2 = self.critic_target(b_s_, action_with_noise, next_actor_hist, next_critic_hist_a)
                target_Q = torch.min(target_Q1, target_Q2)
            ...
            current_Q1,current_Q2 = self.critic(b_s, b_a, b_s_hist_tensor, b_a_hist_tensor)
...
class Memory(object):
    def __init__(self, capacity, dims, pick_len=1):
        self.capacity = capacity
        self.data = np.zeros((capacity, dims))
        self.pointer = 0
        self.exp_rep = ExpRep(capacity, pick_len, False,False,False)
