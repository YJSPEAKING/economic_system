...
OUTPUT = OUTPUT_DIR / "基于模仿学习的复杂经济系统主体决策模型小论文大纲稿_修改版v30.docx"
...
def restructure_chapter_three(document: Document) -> None:
    ...
    anchor = insert_paragraph_after(
        anchor,
        "两个隐藏层共同承担当前状态的特征表示任务。第一隐藏层形成128维中间特征，第二隐藏层进一步形成32维特征，动作输出层据此生成连续经营动作。因此，本章Actor采用基于当前观测的单步状态特征表示。第4章将在该结构基础上引入Transformer编码器，并进一步说明历史状态序列特征与Actor当前状态特征的融合方式。",
        format_source=first_sample_content,
    )
    anchor = insert_paragraph_after(
        anchor,
    ...
    replace_paragraph_text(
        training_intro,
        "基于第3.1节定义的Actor、专家样本和在线经验样本，本文将生成对抗模仿学习引入生产企业主体的TD3训练过程。在该方案中，生产企业Actor同时承担TD3策略网络和GAIL生成器的作用。该方案以经验池中的生产企业交互样本为统一数据基础，使判别器训练、模仿奖励构造和Critic价值估计在同一状态—动作样本接口下衔接，并由Critic的价值估计驱动Actor更新。",
    )
    ...
    replace_paragraph_text(
        ...
        "图3-2给出了基于生成对抗模仿学习与TD3的主体训练方案。在该方案中，专家行为样本为判别器提供专家分布参照，生产企业在线交互样本构成生成样本。判别器根据两类状态-动作样本之间的差异产生模仿奖励。本文将该模仿奖励与环境奖励融合，并将融合奖励用于TD3的Critic目标值计算，使GAIL信号通过价值估计影响Actor更新。模仿学习信号进入策略学习过程的路径可以概括为“判别器—融合奖励—Critic—Actor”。",
    )
    replace_paragraph_text(
        training_caption,
...
def verify(document: Document, source_equation_count: int) -> None:
    ...
    if len(document.inline_shapes) != 12:
        raise RuntimeError(f"Expected 12 inline figures, found {len(document.inline_shapes)}.")
