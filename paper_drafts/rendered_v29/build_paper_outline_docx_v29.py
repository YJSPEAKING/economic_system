...
OUTPUT = OUTPUT_DIR / "基于模仿学习的复杂经济系统主体决策模型小论文大纲稿_修改版v29.docx"
...
def restructure_chapter_three(document: Document) -> None:
    ...
    anchor = insert_paragraph_after(
        new_heading,
        "生产企业主体的决策函数由Actor神经网络实现。Actor采用多层感知机（MLP：Multilayer Perceptron）结构，将生产企业在当前决策时刻的可观测状态映射为连续经营动作。网络输入为经标准化处理的33维状态向量，网络输出为4维动作向量，分别对应贷款意愿（WNDF）、生产资料购买意愿（K）、消费品购买意愿（L）和次日定价（P）。",
    ...
    replace_paragraph_text(
        ...
        "图3-1给出了基于生成对抗模仿学习与TD3的主体训练方案。在该方案中，专家行为样本为判别器提供专家分布参照，生产企业在线交互样本构成生成样本。判别器根据两类状态-动作样本之间的差异产生模仿奖励。本文将该模仿奖励与环境奖励融合，并将融合奖励用于TD3的Critic目标值计算，使GAIL信号通过价值估计影响Actor更新。模仿学习信号进入策略学习过程的路径可以概括为“判别器—融合奖励—Critic—Actor”。",
    )
