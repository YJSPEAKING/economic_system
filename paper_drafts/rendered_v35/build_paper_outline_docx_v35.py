...
def insert_transformer_parameter_table_after(
    ...
    table.autofit = False
    widths = (0.80, 2.15, 1.10, 1.00, 0.95)
...
def update_chapter_four(document: Document) -> None:
    ...
    parameter_description = insert_paragraph_after(
        ...
        "本文将历史序列经过线性映射、Transformer编码和末时刻特征读取后所得向量的维数定义为表征维度。该参数决定历史特征进入后续拼接模块或判别器输出头时的特征规模。三条分支的Transformer编码器均采用单层结构和可学习的位置嵌入，其主要参数设置见表4-1。Actor与Critic分支的表征维度均为32，判别器分支的表征维度为100；三个分支的序列长度均为5，注意力头数均为1。各分支分别维护独立的线性映射、位置嵌入、Transformer编码器和归一化参数，彼此不共享参数。",
        format_source=body_template,
    )
    table_caption = insert_paragraph_after(
    ...
    replace_paragraph_text(
        training_connection,
        "在Transformer+GAIL+TD3的联合训练过程中，Actor、Critic和判别器分别接入基于Transformer的历史序列表征模块。Actor分支将当前状态与历史状态特征拼接后输入独立动作头；Critic分支将当前状态—动作对与历史状态—动作特征拼接后输入双Q值估计分支；判别器分支根据历史状态—动作窗口输出专家概率。奖励融合、Actor-Critic参数更新顺序和环境交互过程采用第3章所述的训练方式。",
    )

    section_heading = find_paragraph(
        document, "4.3 表征增强模块的训练接入与方法边界"
    )
    replace_paragraph_text(section_heading, "4.3 历史表征模块与联合训练")

    method_boundary = find_paragraph(document, "该扩展作用于生产企业主体")
    replace_paragraph_text(
        method_boundary,
        "上述接入方式使历史序列信息分别参与动作生成、价值估计和样本判别。判别器产生的模仿奖励按照第3章定义的融合路径进入Critic目标值，并通过Critic对策略动作的价值估计影响Actor更新，从而将历史序列表征纳入GAIL与TD3的联合训练过程。",
    )
...
def verify(document: Document, source_equation_count: int) -> None:
    ...
    required = (
        ...
        "4.3 历史表征模块与联合训练",
        "本文将历史序列经过线性映射、Transformer编码和末时刻特征读取后所得向量的维数定义为表征维度",
        "本文将历史序列所包含的时间步数定义为序列长度，并记为",
        "可用历史序列的时间步数不足",
