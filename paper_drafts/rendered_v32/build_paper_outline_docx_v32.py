...
OUTPUT = OUTPUT_DIR / "基于模仿学习的复杂经济系统主体决策模型小论文大纲稿_修改版v32.docx"
...
def restructure_chapter_three(document: Document) -> None:
    ...
    replace_paragraph_text(
        training_intro,
        "模型训练通过连续执行多次仿真推进。本文将从环境初始化开始，经过若干天连续运行，直至触发第2.1节所述终止条件的一次完整仿真记为一个回合（episode）。如图3-2所示，每个回合结束后，仿真环境与各主体属性被重置，随后进入下一回合；主体决策模型参数与经验池在回合之间保留，并在后续交互中继续更新。训练过程由此形成日内六阶段运行、回合终止、环境重置与下一回合启动的循环。",
    )
    ...
    replace_paragraph_text(
        ...
        "图3-3给出了基于生成对抗模仿学习与TD3的主体训练方案。在该方案中，专家行为样本为判别器提供专家分布参照，生产企业在线交互样本构成生成样本。判别器根据两类状态-动作样本之间的差异产生模仿奖励。本文将该模仿奖励与环境奖励融合，并将融合奖励用于TD3的Critic目标值计算，使GAIL信号通过价值估计影响Actor更新。模仿学习信号进入策略学习过程的路径可以概括为“判别器—融合奖励—Critic—Actor”。",
    )
    replace_paragraph_text(
        training_caption,
