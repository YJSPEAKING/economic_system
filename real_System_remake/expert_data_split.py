...
EXPERT_TEST_FRACTION = 0.20
EXPERT_SPLIT_SEED = 20260728
...
def load_expert_episode_split(
    csv_path: Path,
    *,
    test_fraction: float = EXPERT_TEST_FRACTION,
    ...
    within either the training or test partition.
    """
    ...
    if unique_episodes.size < 2:
        raise ValueError("At least two complete expert episodes are required for splitting.")

    rng = np.random.default_rng(int(split_seed))
    shuffled = unique_episodes.copy()
    rng.shuffle(shuffled)
    test_episode_count = int(round(unique_episodes.size * test_fraction))
    test_episode_count = min(max(test_episode_count, 1), unique_episodes.size - 1)
    test_episode_ids = np.sort(shuffled[:test_episode_count])
    test_mask = valid_mask & np.isin(episode_ids, test_episode_ids)
    train_mask = valid_mask & ~np.isin(episode_ids, test_episode_ids)
    ...
    metadata: dict[str, Any] = {
        ...
        "train_episodes": int(unique_episodes.size - test_episode_count),
        "test_episodes": int(test_episode_count),
